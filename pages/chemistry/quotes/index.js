...
export default function Quotes() {
    ...
    useEffect(() => {
        fetch(process.env.api + "/chemistry/getQuote/")
            .then(res => res.json())
            .then(data => {
    ...
    const onSave = async () => {
        ...
        // save the quote
        const response = await fetch('/api/chemistry/setQuote', {
            method: 'POST',
            headers: {
    ...
    // clear handler
    const onClear = async () => {
        const response = await fetch('/api/chemistry/clearQuote/', {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    return (
        <>
            <Head>
                <title>Chemistry Quote Editor</title>
            </Head>

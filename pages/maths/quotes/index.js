...
export default function Quotes() {
    ...
    useEffect(() => {
        fetch(process.env.api + "/maths/getQuote/")
            .then(res => res.json())
            .then(data => {
    ...
    const onSave = async () => {
        ...
        // save the quote
        const response = await fetch('/api/maths/setQuote', {
            method: 'POST',
            headers: {
    ...
    // clear handler
    const onClear = async () => {
        const response = await fetch('/api/maths/clearQuote/', {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json'
            },
    ...
    return (
        <>
            <Head>
                <title>Maths Quote Editor</title>
            </Head>

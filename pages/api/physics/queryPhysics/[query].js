...
export default async function handler(req, res) {
    ...
    const types = await prisma.physics_types.findMany({
        where: {
            OR: [
                {
                    main_content: {
    ...
    const theory = await prisma.physics_sos_theory.findMany({
        where: {
            OR: [
                {
                    main_content: {
    ...
    const bookChapters = await prisma.physics_book_chapters.findMany({
        where: {
            OR: [
                {
                    main_content: {
    ...
    const appendix = await prisma.physics_basic_appendix.findMany({
        where: {
            OR: [
                {
                    main_content: {
    ...
    const element = await prisma.physics_element.findMany({
        where: {
            OR: [
                {
                    main_content: {

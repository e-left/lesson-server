...
function SosTheoryForm({ data, newContent: newTheory }) {
    ...
    let originalTheoryJSX = useEditors ? <PhysicsEditor value={originalTheory} onChange={changeOriginalTheory} /> : <></>;
    ...
    const saveCallback = async () => {
        ...
        if (newTheory === "1") {
            // save new word
            const response = await fetch('/api/physics/createPhysicsSosTheory', {
                method: 'POST',
                headers: {
            ...
            window.location.href = "/physics/sostheory";
        } else {
            const response = await fetch('/api/physics/updatePhysicsSosTheory', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },

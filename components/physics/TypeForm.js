...
function TypeForm({ data, newContent: newType }) {
    ...
    let originalTypeJSX = useEditors ? <PhysicsEditor value={originalType} onChange={changeOriginalType} /> : <></>;
    ...
    const saveCallback = async () => {
        ...
        if (newType === "1") {
            // save new word
            const response = await fetch('/api/physics/createPhysicsTypes', {
                method: 'POST',
                headers: {
            ...
            window.location.href = "/physics/types";
        } else {
            const response = await fetch('/api/physics/updatePhysicsTypes', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },

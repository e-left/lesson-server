...
function AppendixForm({ data, newContent: newAppendix }) {
    ...
    let originalAppendixJSX = useEditors ? <PhysicsEditor value={originalAppendix} onChange={changeOriginalAppendix} /> : <></>;
    ...
    const saveCallback = async () => {
        ...
        if (newAppendix === "1") {
            // save new word
            const response = await fetch('/api/physics/createPhysicsAppendix', {
                method: 'POST',
                headers: {
            ...
            window.location.href = "/physics/appendix";
        } else {
            const response = await fetch('/api/physics/updatePhysicsAppendix', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },

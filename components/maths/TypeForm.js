...
function TypeForm({ data, newContent: newType }) {
    ...
    let originalTypeJSX = useEditors ? <MathsEditor value={originalType} onChange={changeOriginalType} /> : <></>;
    ...
    const saveCallback = async () => {
        ...
        if (newType === "1") {
            // save new word
            const response = await fetch('/api/maths/createMathsTypes', {
                method: 'POST',
                headers: {
            ...
            window.location.href = "/maths/types";
        } else {
            const response = await fetch('/api/maths/updateMathsTypes', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },

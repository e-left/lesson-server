...
function AppendixForm({ data, newContent: newAppendix }) {
    ...
    let originalAppendixJSX = useEditors ? <MathsEditor value={originalAppendix} onChange={changeOriginalAppendix} /> : <></>;
    ...
    const saveCallback = async () => {
        ...
        if (newAppendix === "1") {
            // save new word
            const response = await fetch('/api/maths/createMathsAppendix', {
                method: 'POST',
                headers: {
            ...
            window.location.href = "/maths/appendix";
        } else {
            const response = await fetch('/api/maths/updateMathsAppendix', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },

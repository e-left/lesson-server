...
import styles from '../../styles/ProofsElement.module.css';
import Link from 'next/link';

function ProofsElement({ data, initial }) {
    ...
    const returnDeleteCallback = (id) => {
        return async () => {
            const response = await fetch('/api/maths/deleteMathsProofs', {
                method: 'DELETE',
                headers: {
    ...
    if (initial === "1") {
        ...
        const editLink = "/maths/proofs/editProof/" + data.id.toString();
        let taksi = "";
        switch (data.taksh) {
            case 1 :{
                taksi = "Α' Γυμνασίου";

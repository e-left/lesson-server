...
function CurriculumForm({ data, newContent: newCurriculum }) {
    ...
    let originalCurriculumJSX = useEditors ? <MathsEditor value={originalCurriculum} onChange={changeOriginalCurriculum} /> : <></>;
    ...
    const saveCallback = async () => {
        ...
        if (newCurriculum === "1") {
            // save new word
            const response = await fetch('/api/maths/createMathsCurriculum', {
                method: 'POST',
                headers: {
            ...
            window.location.href = "/maths/curriculum";
        } else {
            const response = await fetch('/api/maths/updateMathsCurriculum', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },

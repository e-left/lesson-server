...
function SosTheoryForm({ data, newContent: newTheory }) {
    ...
    let originalTheoryJSX = useEditors ? <MathsEditor value={originalTheory} onChange={changeOriginalTheory} /> : <></>;
    ...
    const saveCallback = async () => {
        ...
        if (newTheory === "1") {
            // save new word
            const response = await fetch('/api/maths/createMathsSosTheory', {
                method: 'POST',
                headers: {
            ...
            window.location.href = "/maths/sostheory";
        } else {
            const response = await fetch('/api/maths/updateMathsSosTheory', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },

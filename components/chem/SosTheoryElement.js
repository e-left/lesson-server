...
import styles from '../../styles/SosTheoryElement.module.css';
import Link from 'next/link';

function SosTheoryElement({ data, initial }) {

    const returnDeleteCallback = (id) => {
        return async () => {
            const response = await fetch('/api/chemistry/deleteChemSosTheory', {
                method: 'DELETE',
                headers: {
    ...
    if (initial === "1") {
        ...
        const editLink = "/chemistry/sostheory/editSosTheory/" + data.id.toString();
        let taksi = "";
        switch (data.taksh) {
            case 1 :{
                taksi = "Α' Γυμνασίου";
...
export default SosTheoryElement;

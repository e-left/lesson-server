...
import styles from '../../styles/CurriculumElement.module.css';
...
function CurriculumElement({ data, initial }) {
    ...
    const returnDeleteCallback = (id) => {
        return async () => {
            const response = await fetch('/api/chemistry/deleteChemCurriculum', {
                method: 'DELETE',
                headers: {
    ...
    if (initial === "1") {
        ...
        const editLink = "/chemistry/curriculum/editCurriculum/" + data.id.toString();
        let taksi = "";
        switch (data.taksh) {
            case 1 :{
                taksi = "Α' Γυμνασίου";

...
function CurriculumForm({ data, newContent: newCurriculum }) {
    ...
    let originalCurriculumJSX = useEditors ? <ChemEditor value={originalCurriculum} onChange={changeOriginalCurriculum} /> : <></>;
    ...
    const saveCallback = async () => {
        ...
        if (newCurriculum === "1") {
            // save new word
            const response = await fetch('/api/chemistry/createChemCurriculum', {
                method: 'POST',
                headers: {
            ...
            window.location.href = "/chemistry/curriculum";
        } else {
            const response = await fetch('/api/chemistry/updateChemCurriculum', {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
